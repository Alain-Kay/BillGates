  ...
  return (
    <div>
      <h2 id="reservation-heading" data-cy="ReservationHeading">
        <Translate contentKey="billGatesApp.reservation.home.title">Reservations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="billGatesApp.reservation.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/reservation/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="billGatesApp.reservation.home.createLabel">Create new Reservation</Translate>
          </Link>
        </div>
      </h2>
        ...
        {reservationList && reservationList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="billGatesApp.reservation.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {reservationList.map((reservation, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{reservation.chambre ? <Link to={`/chambre/${reservation.chambre.id}`}>{reservation.chambre.id}</Link> : ''}</td>
                  <td>{reservation.options ? <Link to={`/options/${reservation.options.id}`}>{reservation.options.id}</Link> : ''}</td>
                  <td>
                    {reservation.gestionnaire ? (
                      <Link to={`/gestionnaire/${reservation.gestionnaire.id}`}>{reservation.gestionnaire.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="billGatesApp.reservation.home.notFound">No Reservations found</Translate>
            </div>
          )
        )}
      </div>
