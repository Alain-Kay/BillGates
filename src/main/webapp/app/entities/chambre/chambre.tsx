  ...
  return (
    <div>
      <h2 id="chambre-heading" data-cy="ChambreHeading">
        <Translate contentKey="billGatesApp.chambre.home.title">Chambres</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="billGatesApp.chambre.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/chambre/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="billGatesApp.chambre.home.createLabel">Create new Chambre</Translate>
          </Link>
        </div>
      </h2>
        ...
        {chambreList && chambreList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="billGatesApp.chambre.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {chambreList.map((chambre, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`billGatesApp.Disponibilite.${chambre.disponibilite}`} />
                  </td>
                  <td>{chambre.images}</td>
                  <td>{chambre.client ? <Link to={`/client/${chambre.client.id}`}>{chambre.client.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="billGatesApp.chambre.home.notFound">No Chambres found</Translate>
            </div>
          )
        )}
      </div>
